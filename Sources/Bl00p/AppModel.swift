...
final class AppModel: ObservableObject {
    ...
    init(
        ...
        if let saved = store.load(), !saved.profiles.isEmpty {
            ...
            managerWorkflows = saved.managerWorkflows.mapValues { workflow in
                ...
                if restored.stage == .verifying {
                    restored.stage = .publishing
                }
            ...
            for workflow in saved.managerWorkflows.values
                where workflow.stage == .verifying {
    ...
    private func advanceWorkflow(
        ...
        case .revising:
            ...
            let reviewSummary = latestAssistantText(for: reviewerID)
            Task { [weak self] in
    ...
    private func prepareBuilderHandoff(
        ...
        do {
            ...
            let lacksPassingTests = package.testStatus != .passed
            guard !lacksRequiredCommit,
                  !hasUncommittedChanges,
                  !lacksPassingTests else {
                ...
                if lacksRequiredCommit {
                    ...
                    reason = "The Builder handoff does not report passing tests."
                }
    ...
    private func dispatchPublishing(
        ...
        guard let package else {
            pauseWorkflow(
                workflow.managerProfileID,
                reason: "The revised Builder handoff is missing."
            )
            return
        }
        guard await resetWorkflowRecipient(
